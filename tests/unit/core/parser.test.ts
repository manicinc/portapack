...
) => Promise<string | Buffer>>(); // Match fs/promises signature
    ...
    describe('🧪 Edge Cases & Robustness', () => {
        it('✅ ignores data URIs', async () => {
         ...
         it('✅ supports malformed or partial tags (best effort by cheerio)', async () => {
             ...
             expectAssetsToContain(result.assets, expected);
             expect(loggerInfoSpy).toHaveBeenCalledWith('HTML parsing complete. Discovered 1 unique asset links.'); // Expect 1
         });
